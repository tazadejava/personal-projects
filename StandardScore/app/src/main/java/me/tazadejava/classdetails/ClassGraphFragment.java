...
public class ClassGraphFragment extends Fragment {

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View parentView = inflater.inflate(R.layout.fragment_class_graph, container, false);

        ClassPeriod period = ((StandardScoreApplication) getActivity().getApplication()).getGradesManager().getCurrentPeriodByIndexAndTerm(getArguments().getInt("classPeriod"), ClassPeriod.GradeTerm.valueOf(getArguments().getString("classTerm")));

        setupGraph(period, (GraphView) parentView.findViewById(R.id.percentageGraph));

        return parentView;
    }

    private void setupGraph(ClassPeriod term, GraphView graph) {

        LineGraphSeries<DataPoint> percentages = new LineGraphSeries<>();

        double minPercent = 100, maxPercent = 0;

        List<PercentageDate> sorted = new ArrayList<>(term.getPercentageDates());
